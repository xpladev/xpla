...
func GetSubmitProposalRegisterZeroRewardValidator() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "register-zero-reward-validator [proposal-file]",
		Args:  cobra.ExactArgs(1),
		Short: "Submit a register zero reward validator proposal",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Submit a register zero reward validator proposal along with an initial deposit.
			The proposal details must be supplied via a JSON file.
			
			Example:
$ %s tx gov submit-proposal register-zero-reward-validator <path/to/proposal.json>

Where proposal.json contains:

{
	"title": "Register zero reward validator proposal",
...
`, version.AppName, xplatypes.DefaultDenom)),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			proposal, err := ParseRegisterZeroRewardValidatorProposalWithDeposit(clientCtx.Codec, args[0])
			if err != nil {
				return err
			}
			...
			content, err := types.NewRegisterZeroRewardValidatorProposal(proposal.Title, proposal.Description, from, sdk.ValAddress(from), pubKey, amount, description)
			if err != nil {
				return err
			}
...
func GetSubmitProposalUnregisterZeroRewardValidator() *cobra.Command {
	bech32PrefixAccAddr := sdk.GetConfig().GetBech32AccountAddrPrefix()

	cmd := &cobra.Command{
		Use:   "unregister-zero-reward-validator [proposal-file]",
		Args:  cobra.ExactArgs(1),
		Short: "Submit a unregister zero reward validator proposal",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Submit a unregister zero reward validator proposal along with an initial deposit.
			The proposal details must be supplied via a JSON file.
			
			Example:
$ %s tx gov submit-proposal unregister-zero-reward-validator <path/to/proposal.json>

Where proposal.json contains:

{
	"title": "Unregister zero reward validator proposal",
...
`, version.AppName, bech32PrefixAccAddr, xplatypes.DefaultDenom)),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			proposal, err := ParseUnregisterZeroRewardValidatorProposalWithDeposit(clientCtx.Codec, args[0])
			if err != nil {
				return err
			}
			...
			content := types.NewUnregisterZeroRewardValidatorProposal(proposal.Title, proposal.Description, valAddr)
