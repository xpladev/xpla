...
function extractChainIDFromGo(goFilePath) {
  try {
    if (!fs.existsSync(goFilePath)) {
      logger.warn(`Go config file not found at ${goFilePath}, using default chain ID: 37`)
      return 37
    }
    ...
    const chainIdMatch = goFileContent.match(/MainnetChainID\s*=\s*(\d+)/)

    if (chainIdMatch) {
      const chainId = parseInt(chainIdMatch[1], 10)
      logger.info(`Extracted MainnetChainID from Go config: ${chainId}`)
      return chainId
    }

    logger.warn('MainnetChainID not found in Go file, using default: 37')
    return 37
  } catch (error) {
    logger.warn(`Error reading Go config file: ${error.message}, using default: 37`)
    return 37
  }
}
...
function syncConfiguration() {
  ...
  const goConfigPath = path.join(__dirname, '../../config/constants.go')
...
function setupNetwork ({ runConfig, timeout }) {
  ...
  // Spawn the xpla process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'xplad started'

    const rootDir = path.resolve(__dirname, '..', '..');     // → ".../xpla"
    const scriptPath = path.join(rootDir, 'local_node.sh');  // → ".../xpla/local_node.sh"

    // Set TEST_SOLIDITY=true for Solidity test environment
    const env = {
      ...process.env,
      TEST_SOLIDITY: 'true'
    }
    ...
    const osdProc = spawn(scriptPath, ['-y'], {
      ...
      env: env  // Pass environment variables including TEST_SOLIDITY
    })

    logger.info(`Starting xplad process... timeout: ${timeout}ms`)
